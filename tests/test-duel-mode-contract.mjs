...
for(const token of [
  ...
  "directSend({kind:'color',color:id})",
  "data?.kind==='color'",
  'function duelColorsConflict()',
  'Both players selected the same color',
  'duelApplySeatColors()',
  'Copy Link',
  'Text / Share Link',
  'scan this QR with the normal Camera app or open the same Clash 4 invite link you send by text'
...
for(const id of ['duelDirectMode','duelPassMode','duelOnlineMode','duelDirectNearby','duelDirectRemote','duelDirectJoin','duelDirectRefreshRow','duelDirectRefreshInvite'])assert.ok(lobby.includes(`id="${id}"`),`Duel hub missing ${id}`);
assert.ok(lobby.includes('id="duelDirectRefreshRow" class="duelAcceptRow" hidden'),'refresh action must be hidden until an invite expires');
...
console.log('PASS Direct Duel + one-scan/refreshable invite + TURN + colors/share + unified Arcade/Direct results + rematch/New Duel split + Pass & Play contracts');