...
function duelPostMatchActionControls(){
  ...
  if(matchMode!=='duel')return;
  if(globalThis.peerRoomRematchActionControls?.(rematchButtons,newMatchButtons))return;
  if(passDuel?.active){
    for(const b of rematchButtons)labelRematch(b)
...
function duelPostMatchStatLabels(){
  const label=statHumanKos?.parentElement?.parentElement?.querySelector?.(':scope > span');if(!label)return;
...
// Direct Duel and Peer Room rematches keep their existing peers; Arcade remains immediate.
document.addEventListener('click',event=>{
  if(matchMode!=='duel')return;
  const rematch=event.target?.closest?.('#restartBottom,#reviewRestart,#sidebarRematch');
  if(rematch){event.preventDefault();event.stopImmediatePropagation();if(globalThis.peerRoomRematchHandleClick?.())return;if(passDuel?.active)globalThis.passRestartMatch?.();else if(directDuel?.active)duelRequestDirectRematch();else globalThis.duelRouteNewDuel?.();return}
  const newDuel=event.target?.closest?.('#homeBottom,#reviewHome,#sidebarHome');
  if(newDuel){event.preventDefault();event.stopImmediatePropagation();if(duelTerminalGuard){clearTimeout(duelTerminalGuard);duelTerminalGuard=null}if(globalThis.peerRoomRematchHandleLobbyClick?.())return;globalThis.duelRouteNewDuel?.()}
},true);
