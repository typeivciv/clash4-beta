...
directChatRender=function(){
  const log=directChatEl('directChatLog'),empty=directChatEl('directChatEmpty'),badge=directChatEl('directChatBadge'),mute=directChatEl('directChatMute'),count=directChatEl('directChatCount'),fab=directChatEl('directChatFab'),input=directChatEl('directChatInput'),send=directChatEl('directChatSend');
  if(!log||!directChatEl('directChatDrawer'))return directChatRenderBeforeV2();
...
directChatSetOpen=function(open,{restoreFocus=true}={}){
  ...
  if(next){
    ...
    requestAnimationFrame(()=>{drawer.classList.add('is-open');if(directChatV2IsMobile()){try{drawer.focus({preventScroll:true})}catch{drawer.focus()}}else{try{directChatEl('directChatInput')?.focus({preventScroll:true})}catch{directChatEl('directChatInput')?.focus()}}});
  }else{
...
function directChatV2Install(){
  ...
  const input=directChatEl('directChatInput'),drawer=directChatEl('directChatDrawer');
  input?.addEventListener('input',directChatRender);
  drawer?.addEventListener('pointerdown',event=>{if(event.target===directChatEl('directChatBackdrop'))directChatClose()});
